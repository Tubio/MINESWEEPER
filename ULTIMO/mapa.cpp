...
void Mapa::eliminarMina(uint fila,uint columna){

	uint posicion = 0;

	Buscador<Mina> buscadorDeMina(this->pMinas);
	buscadorDeMina.buscar(fila,columna,posicion);

	this->pMinas->remover(posicion);
}
