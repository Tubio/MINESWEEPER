...
void Diseniador::crearDisenioBase(){
	...
	unsigned int alto = calcularAlto(this->cantidadFilas); 		//calcula los pixeles de alto
	unsigned int ancho = calcularAncho(this->cantidadColumnas); //calcula los pixeles de ancho

	this-> disenio.SetSize(ancho,alto); //asigna el tamanio

	this->llenarMapaBloqueado(); //crea las casillas
	this->llenarEsquinas();		 //crea las esquinas (estetica)
	this->llenarBordes();		 //crea los bordes (estetica)
...
void Diseniador::escribirDisenio(){
	...
	char nombreDeImpresion[] = "mapa_000"; //nombre base del archivo

	// las siguientes lineas modifican el numero '000' del archivo por el numero actual de impresion
...
void Diseniador::cambiarEnBMP(char valor, uint fila, uint columna){
	...
	BMP tipoDeCasilla; //para almacenar en el programa la casilla nueva
	...
	RangedPixelToPixelCopy(tipoDeCasilla,0,15,15,0,disenio, //cambia la casilla en la posicion por la nueva
